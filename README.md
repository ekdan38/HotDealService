...
<img src="https://github.com/user-attachments/assets/7a592957-6471-4ba6-a418-9ee8e0b7125a" height="100%">
...
&nbsp; HotDeal 프로젝트는 "핫 딜"이라는 이벤트에서 **높은 트래픽과 순간적인 대량의 요청** 상황에서도 **시스템 전체의 안정성과 확장성**을 확보할 수 있는 시스템 구축을 목표로 잡았습니다.

초기 모놀리식 구조는 특정 기능에 부하가 집중되면 저체 서비스가 영향을 받기에 **대규모 동시성 처리 및 확장성**에 한계가 명확했습니다.

따라서 이를 해결하기위해 MSA를 적용하여 다음과 같은 목표를 가졌습니다.

- **서비스 독립성 및 장애 격리**
  - 각 비즈니스 도메인(유저, 상품, 주문, 결제)을 개별 서비스로 분리
  - 특정 서비스 장애가 시스템 전체에 영향을 주지 않도록 구축
...
- **성능 및 효율 개선**
  - 쿼리 최적화 및 인덱스 개선
  - 캐싱 적용으로 조회 성능 향상
  - Redis 기반 분산락으로 재고 일관성 유지 및 동시성 제어

이를 통해 HotDeal 프로젝트는 MSA 아키텍처 적용으로 장애에 강하고 스케일 아웃으로 유연한 대처가 가능하도록 시스템을 구축하고자 했습니다.
