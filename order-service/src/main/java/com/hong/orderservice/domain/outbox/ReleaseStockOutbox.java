...
@Table(name = "release_stock_outbox",
        indexes = {@Index(name = "idx_outbox_status", columnList = "outbox_status")})
public class ReleaseStockOutbox extends TimeEntity {
    @Id
    ...
    private ReleaseStockOutbox(String orderId, Long userId, BigDecimal amount) {
    ...
    // == 생성 메서드 ==
    public static ReleaseStockOutbox create(String orderId, Long userId, BigDecimal amount) {
        return new ReleaseStockOutbox(orderId, userId, amount);
    }
