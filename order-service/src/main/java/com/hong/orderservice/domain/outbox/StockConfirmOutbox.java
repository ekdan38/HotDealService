...
@Getter
@Table(name = "stock_confirm_outbox",
        indexes = {@Index(name = "idx_outbox_status", columnList = "outbox_status")})
public class StockConfirmOutbox extends TimeEntity {
    ...
    private StockConfirmOutbox(String orderId, Long userId, OrderStatus orderStatus) {
    ...
    // == 생성 메서드 ==
    public static StockConfirmOutbox create(String orderId, Long userId, OrderStatus orderStatus) {
        return new StockConfirmOutbox(orderId, userId, orderStatus);
    }
