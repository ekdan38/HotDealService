...
@Table(name = "restore_stock_outbox",
        indexes = {@Index(name = "idx_outbox_status", columnList = "outbox_status")})
public class RestoreStockOutbox extends TimeEntity{
    ...
    private RestoreStockOutbox(String orderId, Long userId, BigDecimal amount) {
    ...
    // == 생성 메서드 ==
    public static RestoreStockOutbox create(String orderId, Long userId, BigDecimal amount) {
        return new RestoreStockOutbox(orderId, userId, amount);
    }
